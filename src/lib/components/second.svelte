<script lang="ts">
  ...
  function openSkill(skill: Skill) { selectedSkill = skill; }
  function closeModal() { selectedSkill = null; }
...
<div class="flex flex-col justify-items-center items-start w-full lg:items-center p-4">
  ...
  <div class="justify-start lg:justify-center items-center lg:place-items-center flex flex-col mt-2 mb-2 lg:w-1/2 animate-fade-in-up">
    <p class="font-extrabold text-2xl lg:text-4xl text-slate-900">
      Helping young Africans turn ideas into <b class="text-rose-600 font-extrabold">real products and businesses</b> that create lasting impact.
    </p>
    <p class="text-slate-500 text-sm lg:text-base animate-fade-in-delay mt-3">
  ...
  <!-- Skills Set -->
  <div class="text-slate-900 shadow-sm bg-white border border-slate-100 rounded-2xl p-6 mt-6 w-full lg:w-1/2 animate-fade-in-up">
    <div class="shadow-sm shadow-rose-200 rounded-2xl p-2 m-2 mb-6 text-center border border-slate-100">
      <p class="font-extrabold text-xl lg:text-3xl text-rose-600 pb-4">Skills & Credentials</p>
      <span class="text-slate-500 font-normal text-xs">Click on a skill to view proof</span>
    </div>
    ...
    <div class="flex flex-wrap gap-4 justify-start lg:justify-center">
      {#each skills as skill}
        <button
          ...
          class="bg-slate-100 px-3 py-1.5 lg:px-4 lg:py-2 rounded-full text-xs lg:text-lg
                 hover:bg-slate-200 hover:scale-105 transition-all duration-300
                 animate-fade-in-delay cursor-pointer border border-slate-200 hover:border-rose-600 text-slate-800"
        >
          {skill.name}
        </button>
      {/each}
    ...
    <!-- About Me -->
    <div class="text-left p-4 lg:p-6 text-slate-700 text-sm lg:text-base mt-6 bg-slate-50 rounded-2xl border border-slate-100 animate-fade-in-up space-y-4">
      <h1 class="font-extrabold text-xl lg:text-3xl text-rose-600 text-center">About Me</h1>
...
{#if selectedSkill}
  <div
    ...
    class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in-up"
  ...
  >
    <div class="bg-white rounded-2xl p-6 max-w-md w-full shadow-xl border border-slate-200">
      <div class="flex justify-between items-center mb-4">
        <p id="modal-title" class="text-2xl font-extrabold text-rose-600">{selectedSkill.name}</p>
        <button on:click={closeModal} class="text-slate-400 hover:text-slate-900 text-2xl">×</button>
      </div>
      <div class="space-y-3">
        {#each selectedSkill.proofs as proof}
          <a
            ...
            class="block bg-slate-50 hover:bg-slate-100 p-4 rounded-lg transition-colors duration-300 border border-slate-200 animate-fade-in-delay"
          >
            <p class="text-slate-900 font-semibold">{proof.title}</p>
            <p class="text-slate-500 text-sm">View proof →</p>
          </a>
        {/each}
      </div>
    </div>
